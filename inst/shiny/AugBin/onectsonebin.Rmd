...
The joint likelihood contribution for patient i with for instance, $Y_{i2}=0$, can be factorised as shown below

$$\begin{equation}
l\left(\boldsymbol{\theta};\mathbf{Y_{i}^{*}}\right)=f(Y_{i1}^{};\boldsymbol{\theta})\int_{-\infty}^{0}f(Y_{i2}^{*}|Y_{i1}^{};\boldsymbol{\theta})dy_{2}^{*}
\end{equation}$$
